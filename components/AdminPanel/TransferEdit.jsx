...
const TransferEdit = ({ editingId, changeDetails, setEditing }) => {
  ...
  const formHandler = async ({ date, amount, email, status }) => {
    console.log(date, amount, email, status);
    console.log(editingId);
    document.getElementById("myForm").reset();

    try {
      const { data } = await axios.put(`/api/transactions/editTransfers`, {
        amount,
        status,
        editingId,
        date,
      ...
      console.log(data);
      changeDetails(data.toUpdateTransfer);
    } catch (error) {
      console.log(error);
    }
  };
