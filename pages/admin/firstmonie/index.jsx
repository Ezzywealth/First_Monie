...
const FirstmonieAdmin = ({ users, isAdmin }) => {
	...
	const isAdminSidebarOpen = useSelector((state) => state?.generalSlice?.isAdminSidebarOpen);
	...
	return (
		<div className='relative h-screen overflow-auto bg-indigo-50 w-full  gap-0  grid grid-cols-1 md:grid-cols-4 '>
			<div className={`fixed z-50 customTransition col-span-1 ${isAdminSidebarOpen ? 'h-screen' : 'hidden -left-[1000px]'}`}>
				<AdminSidebar />
			</div>
			...
			<div className='col-span-3 relative h-screen overflow-auto'>
				<main className='relative'>
					...
					<h2 className='font-bold text-gray-500 px-8 text-2xl mb-4'>Dashboard</h2>
					<section className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 px-8 mb-8'>
						{adminDashboardLists.map((item) => (
							<div className='bg-white px-3 flex justify-between items-center py-8 rounded-lg' key={item.id}>
								<div className='flex flex-col'>
									<span className='text-[12px] text-gray-400 font-semibold'>{item.title}</span>
					...
					<section className='flex flex-col mx-2 md:mx-10 space-y-4 mb-16  lg:mx-8 bg-white py-4 border border-gray-300 border-solid'>
						<div className='flex justify-between items-center px-4'>
							...
							<button className='bg-green-500 px-3 py-1 rounded-lg text-white hover:scale-105 customTransition' onClick={() => router.push('/admin/firstmonie/createNewUser')}>
								Add User
							</button>
						</div>
							...
							<table className='min-w-full overflow-auto w-[600px] table-auto'>
								...
								<tbody>
									{newUsers?.map((user, index) => (
										<tr key={user._id} className='border-b border-solid border-gray-200 text-[14px] gap-4 '>
											...
											<td
												...
												<button className='bg-indigo-500 hover:scale-105 hover:bg-indigo-700 customTransition text-white px-3 py-1 rounded-lg'>Details/edit user</button>
											</td>
											<td
												className='text-center'
												onClick={() => {
												...
												<button className='bg-indigo-500 hover:scale-105 hover:bg-indigo-700 customTransition text-white px-3 py-1 rounded-lg'>Delete</button>
											</td>
										</tr>
									))}
								</tbody>
...
export async function getServerSideProps(ctx) {
	...
	return {
		props: {
			...
			users: users.map(db.convertUsersDocToObj).filter((user) => user.isAdmin !== true),
		},
	};
}
